...
import {
  Database, BarChart3, FileSpreadsheet, Code, Server, GitBranch,
  Sparkles, Zap, Brain, Layout, Palette, Cloud, Wand2, Network, FileText,
  Settings, CheckSquare, Smartphone, Layers,
} from "lucide-react";
import {
  SiJavascript, SiTypescript, SiPython, SiPostgresql, SiMysql,
  SiReact, SiAngular, SiMui, SiTailwindcss, SiSass, SiHtml5, SiGreensock,
  SiNodedotjs, SiExpress, SiFlask, SiSpringboot,
  SiMongodb, SiSupabase,
  SiGooglecloud, SiGithub, SiNetlify, SiSwagger, SiCpanel, SiRender,
  SiGit, SiAnthropic,
} from "react-icons/si";
...
const TableauIcon = ({ size = 18, className = "" }: { size?: number; className?: string }) => (
...
const PowerBIIcon = ({ size = 18, className = "" }: { size?: number; className?: string }) => (
...
const DashboardIcon = ({ size = 18, className = "" }: { size?: number; className?: string }) => (
  <svg width={size} height={size} viewBox="0 -0.5 25 25" fill="none" className={className} aria-hidden="true">
...
type AnyIcon = IconType | LucideIcon | (({ size, className }: { size?: number; className?: string }) => React.ReactElement);
...
const catIconMap: Record<string, LucideIcon> = {
  Database, BarChart3, FileSpreadsheet, Code, Server, GitBranch,
  Sparkles, Zap, Brain, Layout, Palette, Cloud, Wand2, Network,
  FileText, Settings, CheckSquare, Smartphone, Layers,
};
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0">
          {skillCategories.map((category, idx) => {
            ...
            const CatIcon = catIconMap[category.icon as keyof typeof catIconMap];
              ...
              >
                ...
                <ul className="space-y-3">
                  {category.skills.map((skill, index) => {
                    const SkillIcon = skillIconMap[skill.name] as AnyIcon | undefined;
